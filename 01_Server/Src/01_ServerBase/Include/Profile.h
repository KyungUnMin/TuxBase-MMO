...
#define FUNCTION_PROFILE()                                                     \
  common::ProfileSampler sampler##__LINE__(TUXBASE_FUNCTION_NAME);

#define BLOCK_PROFILE(name)                                                    \
  if (bool once = true)                                                        \
    for (common::ProfileSampler sampler##__LINE__(name); once; once = false)

#define PROFILE_SAMPLE_START(fileName, bCachedProfile)                         \
  common::ProfileStart(fileName, bCachedProfile)
#define PROFILE_SAMPLE_END() common::ProfileEnd()
...
public:
  template <typename T> ProfileSampler(const T *) = delete;
