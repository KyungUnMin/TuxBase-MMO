...
- 코드 작업 중 포트폴리오에 어필할 만한 요소(Clean Architecture 계층 분리 적용, 의미 있는 디자인 패턴 도입, 스마트 포인터 기반 소유권 설계, 흥미로운 트레이드오프 결정 등)를 발견하거나 직접 적용했다면, `01_Server/Doc/DevNotes/Highlights/`에 가독성 있는 `.md` 문서로 정리합니다. 작업 중 자연스럽게 눈에 띄면 먼저 제안하고, 사용자가 동의하면 작성합니다. 문서에는 최소한 다음을 포함합니다: 어떤 문제/상황이었는지, 어떤 선택을 했는지, 트레이드오프(왜 그 대안을 골랐는지), 관련 코드 위치(`file:line`).
...
- 채팅 서버는 독립된 별도 프로세스로 분리하고, **C#(.NET)**으로 만들 수도 있습니다. 아직 착수 전이며 확정된 계획은 아닙니다. C++ 엔진(`02_ServerEngine`)과는 별개 빌드 체계(`dotnet` CLI, CMake 아님)와 별도 Docker 이미지로 구성합니다.
